...
func (q Query) ToSQL(model *Model, addColumns ...string) (string, []interface{}) {
	sb := q.toSQLBuilder(model, q.Connection.Dialect.Name(), addColumns...)
...
func (q Query) toSQLBuilder(model *Model, dialectName string, addColumns ...string) *sqlBuilder {
	if len(q.addColumns) != 0 {
		addColumns = q.addColumns
	}
	return newSQLBuilder(q, model, dialectName, addColumns...)
}
